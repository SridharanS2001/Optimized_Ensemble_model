...
Original file is located at
    https://colab.research.google.com/drive/1C3y9SSQbxLpV8GcPNdMrBG9Mdlj7v5F2
...
model_definitions = {
    'SVR': Pipeline([
        ...
        ('model', SVR(C=150.2053834224418, epsilon=1.0, gamma=0.001218266700169731, kernel='rbf'))
    ]),
    'RandomForest': Pipeline([
        ('scaler', StandardScaler()),
        ('model', RandomForestRegressor(bootstrap=False, max_depth=17, min_samples_leaf=2,
                                        min_samples_split=4, n_estimators=167, random_state=42))
    ]),
    'ExtraTrees': Pipeline([
        ('scaler', StandardScaler()),
        ('model', ExtraTreesRegressor(bootstrap=False, max_depth=5, min_samples_leaf=1,
                                      min_samples_split=2, n_estimators=300, random_state=42))
    ]),
    'GaussianProcess': Pipeline([
        ('scaler', StandardScaler()),
        ('model', GaussianProcessRegressor(alpha=0.081183958953274, n_restarts_optimizer=10))
    ]),
    'KNN': Pipeline([
        ('scaler', StandardScaler()),
...
optimal_weights = [0.0000, 0.0000, 0.0000, 1.0000, 0.0000]
...
plot_data.to_csv('Mm.csv', index=False)
...
optimal_weights = [0.0000, 0.0000, 1.0000, 0.0000, 0.0000]
...
optimal_weights = [0.0000, 0.0000, 1.0000, 0.0000, 0.0000]
knn_weight, svr_weight, rfr_weight, dt_weight, xgb_weight = optimal_weights
...
generate_combinations_and_save(['Ni', 'Ti', 'Zr'], 'Ni_Ti_Zr_combinationsM.csv')
generate_combinations_and_save(['Ni', 'Ti', 'Hf'], 'Ni_Ti_Hf_combinationsM.csv')
generate_combinations_and_save(['Ni', 'Ti', 'Pd'], 'Ni_Ti_Pd_combinationsM.csv')
generate_combinations_and_save(['Ni', 'Ti', 'Pt'], 'Ni_Ti_Pt_combinationsM.csv')